...
async function main() {
  ...
  const silverTasks = [
    ...
    { slug: 'word-count', title: 'Счётчик слов', statement: 'Количество слов в строке.', signature: 'solution(s: str) -> int', args: ['s'], example: { input: '"hello world"', output: '2' }, tests: [{ args: ['hello world python'], expected: '3' }, { args: ['one'], expected: '1' }] },
    ...
    { slug: 'merge-sorted', title: 'Слияние', statement: 'Слейте отсортированные.', signature: 'solution(a: list, b: list) -> list', args: ['a', 'b'], example: { input: '[1,3,5], [2,4,6]', output: '[1,2,3,4,5,6]' }, tests: [{ args: [[1, 3, 5], [2, 4, 6]], expected: '[1, 2, 3, 4, 5, 6]' }] },
    { slug: 'difference', title: 'Разность', statement: 'Элементы a без b.', signature: 'solution(a: list, b: list) -> list', args: ['a', 'b'], example: { input: '[1,2,3,4], [2,4]', output: '[1, 3]' }, tests: [{ args: [[1, 2, 3, 4], [2, 4]], expected: '[1, 3]' }] },
    { slug: 'intersection', title: 'Пересечение', statement: 'Общие элементы.', signature: 'solution(a: list, b: list) -> list', args: ['a', 'b'], example: { input: '[1,2,3], [2,3,4]', output: '[2, 3]' }, tests: [{ args: [[1, 2, 3], [2, 3, 4]], expected: '[2, 3]' }] },
    { slug: 'most-frequent', title: 'Частый элемент', statement: 'Самый частый.', signature: 'solution(lst: list) -> any', args: ['lst'], example: { input: '[1,2,2,3,2]', output: '2' }, tests: [{ args: [[1, 2, 2, 3, 2]], expected: '2' }] },
  ...
  const goldTasks = [
    ...
    { slug: 'roman-to-int', title: 'Римское→Число', statement: 'Римское число в int.', signature: 'solution(s: str) -> int', args: ['s'], example: { input: '"XIV"', output: '14' }, tests: [{ args: ['XIV'], expected: '14' }, { args: ['III'], expected: '3' }] },
    { slug: 'valid-brackets', title: 'Скобки', statement: 'Валидны ли ()[]{}?', signature: 'solution(s: str) -> bool', args: ['s'], example: { input: '"()[]{}"', output: 'True' }, tests: [{ args: ['()[]{}'], expected: 'True' }, { args: ['([)]'], expected: 'False' }] },
    ...
    { slug: 'max-subarray', title: 'Макс подсумма', statement: 'Макс сумма подмассива.', signature: 'solution(nums: list) -> int', args: ['nums'], example: { input: '[-2,1,-3,4,-1,2,1,-5,4]', output: '6' }, tests: [{ args: [[-2, 1, -3, 4, -1, 2, 1, -5, 4]], expected: '6' }] },
    { slug: 'merge-intervals', title: 'Слияние интервалов', statement: 'Объедините пересечения.', signature: 'solution(intervals: list) -> list', args: ['intervals'], example: { input: '[[1,3],[2,6],[8,10]]', output: '[[1,6],[8,10]]' }, tests: [{ args: [[[1, 3], [2, 6], [8, 10]]], expected: '[[1, 6], [8, 10]]' }] },
    { slug: 'group-anagrams', title: 'Группы анаграмм', statement: 'Сгруппируйте анаграммы.', signature: 'solution(strs: list) -> list', args: ['strs'], example: { input: '["eat","tea","ate"]', output: '[["ate","eat","tea"]]' }, imports: ['itertools'], tests: [{ args: [['eat', 'tea', 'ate']], expected: "[['ate', 'eat', 'tea']]" }] },
    ...
    { slug: 'count-primes', title: 'Счёт простых', statement: 'Простых меньше n.', signature: 'solution(n: int) -> int', args: ['n'], example: { input: '10', output: '4' }, tests: [{ args: [10], expected: '4' }, { args: [2], expected: '0' }] },
    ...
    { slug: 'count-say', title: 'Посчитай-скажи', statement: 'N-й элемент последовательности.', signature: 'solution(n: int) -> str', args: ['n'], example: { input: '4', output: '"1211"' }, imports: ['itertools'], tests: [{ args: [1], expected: '1' }, { args: [4], expected: '1211' }] },
    { slug: 'reverse-bits', title: 'Биты наоборот', statement: 'Переверните 32 бита.', signature: 'solution(n: int) -> int', args: ['n'], example: { input: '43261596', output: '964176192' }, tests: [{ args: [43261596], expected: '964176192' }] },
    { slug: 'atoi', title: 'String to Int', statement: 'Строку в число (как atoi).', signature: 'solution(s: str) -> int', args: ['s'], example: { input: '"42"', output: '42' }, tests: [{ args: ['42'], expected: '42' }, { args: ['   -42'], expected: '-42' }] },
    { slug: 'eval-rpn', title: 'RPN калькулятор', statement: 'Вычислите RPN.', signature: 'solution(tokens: list) -> int', args: ['tokens'], example: { input: '["2","1","+","3","*"]', output: '9' }, imports: ['functools'], tests: [{ args: [['2', '1', '+', '3', '*']], expected: '9' }] },
  ];
