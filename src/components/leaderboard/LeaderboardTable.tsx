  ...
  return (
    <div className="space-y-3">
      ...
      <div className="md:hidden space-y-2">
        {displayEntries.length === 0 ? (
          <div className="card px-4 py-8 text-center text-text-muted">{emptyMessage}</div>
        ) : (
          displayEntries.map((entry) => (
            <div
              key={entry.rank}
              className={cn(
                'card px-3 py-2.5',
                entry.isCurrentUser && 'border-accent-blue/40 bg-accent-blue/5'
              )}
            >
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-right">
                  <div className="font-mono font-bold text-accent-green">{entry.codeLength}</div>
